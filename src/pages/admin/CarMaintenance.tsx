                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (!pw) return;
                                  pw.document.write(`<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>تقرير مالي - ${car.car_number}</title><style>@import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap');body{font-family:'Cairo',sans-serif;padding:30px;color:#1e293b}h1{font-size:22px}h2{font-size:16px;color:#475569;margin-bottom:20px}table{width:100%;border-collapse:collapse;margin-top:15px}th{background:#f1f5f9;padding:10px;text-align:right;border-bottom:2px solid #cbd5e1;font-size:13px}td{padding:10px;border-bottom:1px solid #e2e8f0;font-size:13px}.summary{display:flex;gap:20px;margin:20px 0}.summary div{flex:1;padding:15px;border-radius:12px;text-align:center}.rev{background:#ecfdf5;color:#059669}.exp{background:#fef2f2;color:#dc2626}.net{background:#eef2ff;color:#4f46e5}@media print{body{padding:0}}</style></head><body><h1>تقرير مالي - سيارة ${car.car_number}</h1><h2>${car.car_details} | العميل: ${car.customer_name}</h2><div class="summary"><div class="rev"><b>الإيرادات</b><br/>${totalRevenue.toFixed(2)} ج.م</div><div class="exp"><b>المصروفات</b><br/>${totalExpense.toFixed(2)} ج.م</div><div class="net"><b>صافي الربح</b><br/>${netProfit.toFixed(2)} ج.م</div></div><table><thead><tr><th>التاريخ</th><th>النوع</th><th>المبلغ</th><th>طريقة الدفع</th><th>البيان</th></tr></thead><tbody>${allTx.map(t => `<tr><td>${t.date}</td><td>${t.type}</td><td>${t.amount} ج.م</td><td>${t.method === 'cash' ? 'كاش' : t.method === 'visa' ? 'فيزا' : t.method === 'wallet' ? 'محفظة' : 'انستا باي'}</td><td>${t.note}</td></tr>`).join('')}</tbody></table><script>window.onload=()=>{window.print();setTimeout(()=>window.close(),500)}</script></body></html>`);
                                  pw.document.close();
                                }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-2xl relative">
            ...
            <form onSubmit={async (e) => {
              ...
              const pw = window.open('', '_blank');
              if (pw) {
                pw.document.write(`<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>إيصال سداد - ${car.car_number}</title><style>@import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap');body{font-family:'Cairo',sans-serif;padding:30px;text-align:center;color:#1e293b}h1{color:#10b981;margin-bottom:10px}p{font-size:18px;margin:5px 0}.qr{margin-top:30px}</style></head><body><h1>إيصال سداد مجمع</h1><p>العميل: ${car.customer_name}</p><p>رقم السيارة: ${car.car_number}</p><p>المبلغ المسدد: ${totalAmount} ج.م</p><img class="qr" src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${qrData}" /><script>window.onload=()=>{window.print();setTimeout(()=>window.close(),500)}</script></body></html>`);
                pw.document.close();
              }
            }} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
