...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'request') {
      ...
      const { error } = await admin.from('otp_codes').insert({ code, purpose: 'wholesale', expires_at: expires, used: false });
      if (error) { res.status(500).json({ ok: false, error: error.message }); return; }
      try {
        await sendTelegramText(`🔐 رمز تأكيد فاتورة الجملة / نص الجملة:\n\n${code}\n\nصالح لمدة 5 دقائق. لا تشاركه إلا مع من يحق له البيع بالجملة.`);
      } catch (e) {
